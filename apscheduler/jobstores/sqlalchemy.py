...
try:
    from sqlalchemy import (
        create_engine, Table, Column, MetaData, Unicode, Float, LargeBinary, select, Integer, and_)
...
class SQLAlchemyJobStore(BaseJobStore):
    ...
    def get_next_run_time(self, now=None):
        if now:
            # selectable = select([self.jobs_t.c.next_run_time]).where(self.jobs_t.c.next_run_time >= now).order_by(self.jobs_t.c.next_run_time).limit(1)
    ...
    def cas_update_job(self, job):
        update = self.jobs_t.update().values(**{
            'next_run_time': datetime_to_utc_timestamp(job.next_run_time),
            'job_state': pickle.dumps(job.__getstate__(), self.pickle_protocol),
            'version':job.version + 1
        }).where(and_(self.jobs_t.c.id == job.id, self.jobs_t.c.version == job.version))
        result = self.engine.execute(update)
    ...
    def _reconstitute_job(self, job_state, version):
        job_state = pickle.loads(job_state)
        job_state['jobstore'] = self
        job = Job.__new__(Job)
        job.__setstate__(job_state, version)
    ...
    def _get_jobs(self, *conditions):
        jobs = []
        selectable = select([self.jobs_t.c.id, self.jobs_t.c.job_state, self.jobs_t.c.version]).\
            order_by(self.jobs_t.c.next_run_time)
        selectable = selectable.where(*conditions) if conditions else selectable
        failed_job_ids = set()
        for row in self.engine.execute(selectable):
            try:
                jobs.append(self._reconstitute_job(row.job_state, row.version))

            ...
            for jobstore_alias, jobstore in six.iteritems(self._jobstores):
                ...
                for job in due_jobs:
                    ...
                    if run_times:
                        ...
                        if jobstore.cas_update_job(job):
                            try:
                            ...
                            self._logger.error('the job has been executed by other instance')
                            # job_next_run = job.trigger.get_next_fire_time(run_times[-1], now)
                            # if job_next_run:
                            #     job._modify(next_run_time=job_next_run)
                            #     if not jobstore.cas_update_job(job):
                            #
                            #     # jobstore.update_job(job)
                            # else:
                            #     self.remove_job(job.id, jobstore_alias)
                ...
                jobstore_next_run_time = jobstore.get_next_run_time(now)
